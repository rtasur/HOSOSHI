...
if not exist ".env" (
    echo No .env found. Creating local configuration from .env.example...
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\prepare-local-env.ps1"
if errorlevel 1 (
    echo ERROR: Could not prepare local environment.
    pause
    exit /b 1
)
...
if errorlevel 1 (
    ...
    echo.
    echo Review the errors above. The application will NOT be opened until the backend health check passes.
    pause
    exit /b 1
)
...
echo.
echo ============================================================
echo HOSOSHI STARTUP TIMED OUT
echo ============================================================
echo.
echo Backend/frontend did not become reachable within 120 seconds.
echo This is usually a database startup or backend configuration issue.
echo.
docker compose ps
echo.
echo ---- Backend log ----
