...
export default function CasesPage() {
  ...
  const [assignRole, setAssignRole] = useState<'SUPERVISOR' | 'INVESTIGATOR'>('SUPERVISOR');
  ...
  const loadAssignable = async () => {
    ...
    const filtered = (r.data as AssignableUser[]).filter(x => {
      ...
      if (isSupervisor) return x.roles?.includes('INVESTIGATOR');
      return x.roles?.includes('INVESTIGATION_SUPERVISOR') || x.roles?.includes('INVESTIGATOR');
    });
    setAssignable(filtered);
    if (filtered.length) {
      setSelectedUser(filtered[0].id);
      setAssignRole(isSupervisor ? 'INVESTIGATOR' : (filtered[0].roles.includes('INVESTIGATION_SUPERVISOR') ? 'SUPERVISOR' : 'INVESTIGATOR'));
    } else {
      setSelectedUser('');
    }
  };
  ...
  const onAssignUserChange = (id: string) => {
    ...
    if (!isSupervisor && target) {
      setAssignRole(target.roles.includes('INVESTIGATION_SUPERVISOR') ? 'SUPERVISOR' : 'INVESTIGATOR');
    }
  };
  ...
  return <div>
    ...
    {assignCase && <div className="modal-backdrop"><div className="modal">
      ...
      <label>Case role<select value={assignRole} onChange={e => setAssignRole(e.target.value as 'SUPERVISOR' | 'INVESTIGATOR')} disabled={isSupervisor}>{isSupervisor ? <option value="INVESTIGATOR">INVESTIGATOR</option> : <><option value="SUPERVISOR">SUPERVISOR</option><option value="INVESTIGATOR">INVESTIGATOR</option></>}</select></label>
      <div className="notice"><span>AUTHORITY</span><span>{isSupervisor ? 'You may delegate cases under your authority to Investigators.' : 'Administrators may assign cases to Investigation Supervisors or Investigators.'}</span></div>
