...
public class CaseService {
    ...
    public List<CaseResponse> list(Authentication a, String sort, String priority) {
        ...
        if (hasRole(u, RoleCode.SUPER_ADMIN, RoleCode.AUDITOR)) {
            visible = all;
    ...
    public List<CaseResponse> bin(Authentication a) {
        var u = currentUser(a);
        requireRole(u, RoleCode.SUPER_ADMIN);
    ...
    public CaseResponse create(CreateCaseRequest r, Authentication a) {
        var u = currentUser(a);
        requireRole(u, RoleCode.SUPER_ADMIN, RoleCode.INVESTIGATION_SUPERVISOR);
        var now = Instant.now();
    ...
    public void assign(UUID id, AssignRequest r, Authentication a) {
        ...
        if (hasRole(u, RoleCode.INVESTIGATION_SUPERVISOR)) {
            assertAccess(c, a);
            if (!Set.of(CaseMemberRole.INVESTIGATOR, CaseMemberRole.ANALYST, CaseMemberRole.DATA_OPERATOR).contains(r.memberRole())) {
                throw new AccessDeniedException("Investigation Supervisors can assign Investigators, Analysts or Data Operators");
            }
        } else if (!Set.of(CaseMemberRole.SUPERVISOR, CaseMemberRole.INVESTIGATOR,
                CaseMemberRole.ANALYST, CaseMemberRole.DATA_OPERATOR).contains(r.memberRole())) {
            throw new IllegalArgumentException("Unsupported case assignment role");
        }
    ...
    public CaseResponse update(UUID id, UpdateCaseRequest r, Authentication a) {
        ...
        requireRole(actor, RoleCode.SUPER_ADMIN, RoleCode.INVESTIGATION_SUPERVISOR);
        assertAccess(c, a);
    ...
    public void delete(UUID id, Authentication a) {
        var actor = currentUser(a);
        requireRole(actor, RoleCode.SUPER_ADMIN);
        var c = activeCase(id);
        var now = Instant.now();
        c.setDeletedAt(now);
    ...
    public void removeMember(UUID id, UUID userId, Authentication a) {
        ...
        if (hasRole(actor, RoleCode.INVESTIGATION_SUPERVISOR)) {
            ...
            if (!hasRole(target, RoleCode.INVESTIGATOR, RoleCode.INTELLIGENCE_ANALYST, RoleCode.DATA_OPERATOR)) {
                throw new AccessDeniedException("Investigation Supervisors can remove only operational case members they supervise");
            }
        }
    ...
    private void assertAccess(InvestigationCase c, Authentication a) {
        if (c.getDeletedAt() != null) throw new NoSuchElementException("Case is in Bin");
        var u = currentUser(a);
        if (hasRole(u, RoleCode.SUPER_ADMIN, RoleCode.AUDITOR)) return;
        if (hasRole(u, RoleCode.INVESTIGATION_SUPERVISOR)) {
